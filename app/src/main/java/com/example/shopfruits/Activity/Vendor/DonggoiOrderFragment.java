...
import com.example.shopfruits.databinding.FragmentDonggoiBinding;
...
public class DonggoiOrderFragment extends Fragment {
    FragmentDonggoiBinding binding;
    RecyclerView DonHang;
    DonHang_ShopAdapter donHangAdapter;
    APIService apiService;

    public DonggoiOrderFragment(){}
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
        binding= FragmentDonggoiBinding.inflate(inflater, container,false);
        DonHang=binding.rcDonhangDonggoi;
        int id = SharePrefManager.getInstance(getActivity()).getStoreID();
        apiService= RetrofitClient.getInstance().getRetrofit(constants.ROOT_URL).create(APIService.class);
        apiService.GetTrangThai(id,"Đã Đóng Gói").enqueue(new Callback<List<DonHang_Shop_Model>>() {
            @Override
